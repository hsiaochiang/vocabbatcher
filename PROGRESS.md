...
- **目前階段：** ③ 開發（垂直切片）— M3-S1「發音按鈕全面化」已驗收通過，準備開 S2
- **整體進度：** ▓▓▓▓▓▓▓░░░ ~62%（M1、M2 完成；M3-S1 完成並驗收通過；正式環境已上線；S2~S5 未實作；M4 未開始）
- **最後更新：** 2026-07-30 by 執行層

---

## ▶ 下一步就做這個（回來先看這裡）

**開 `BRIEF_S2_雲端帳號.md`**（Google 登入 + Firebase 雲端基礎）。開工前置條件：Wilson 需先到 console.firebase.google.com 建立 Firebase 專案並啟用 Google 登入 + Firestore，取得 `firebaseConfig` 後交給執行層（詳見 BRIEF_S2 前置條件）。

## ⏳ 等你決定

- Firebase 專案何時建立？（約 10 分鐘，S2 BRIEF 已附白話步驟）
...
- M3「考試與追蹤」：S1 已完成並驗收通過；S2~S5 切片依序待實作（見 `STAGE_3_PLAN.md`）
