...
- **目前階段：** 🎉 **M1~M5 全部里程碑驗收通過並穩定上線**，另加學測 Minecraft 故事模式（新功能）已完成開發並部署上線。成果文件交付閘已補齊（`USER_MANUAL.md`／`DEVELOPER_LOG.md` 涵蓋至 M5，尚未補故事模式章節）；2026-08-13 修復三個部署/前端 bug，Firebase Hosting 與 GitHub Pages 備援兩邊皆已重新部署驗證正常
- **整體進度：** ▓▓▓▓▓▓▓▓▓▓ 100%（M1~M5 全部完成並驗收通過；學測故事模式為 M5 之後新增的功能，已上線，目前為穩定維運狀態）
...
**學測 Minecraft 故事模式已本機驗證通過、push + 部署到 Firebase Hosting 與 GitHub Pages 完成。**
...
- 切片2b（剩餘75頁，重做版）：**已通過品質審查**——執行層改成把每句故事明文寫在資料檔，規劃層獨立驗證（576句句型骨架無重複、`wordList` 與正式資料完全一致、抽查未指定頁面內容自然）後確認過關；179句超過「每句最多3字」上限，負責人看過例句後接受現狀不需重做（詳見 `DECISIONS.md` 2026-08-13「切片2b 重做通過但密度超出上限」條目）。
- 切片3（App 端串接）：**已完成並通過驗收。** 頁碼快速建立的學測批次會記住來源頁碼（`Batch.sourcePage`），若該頁有 Minecraft 故事，批次 Hub 會出現「故事模式」入口，能讀英文/中文對照、點句子或中文對照裡的英文單字都能發音；手動勾字批次不會出現故事入口。規劃層核對程式碼 diff（型別、路由、快取規則）與 UI 截圖，確認實作精準對應規格、未誤觸當天稍早修好的敏感邏輯（`BrowserRouter`/`setSource`）。負責人已在本機 `vite preview` 環境實際操作驗證通過。
- **2026-08-14：負責人本機驗證完成，規劃層已 `git push` 並跑 `.\deploy.ps1` 部署到 Firebase Hosting（`https://gen-lang-client-0930375434.web.app`），GitHub Pages 備援管道也已因 push 自動觸發部署。學測 Minecraft 故事模式（第16~20頁人工試行 + 第1~15/21~80頁執行層生成，共80頁）正式上線。**

## ⏳ 等你決定（新增）

- 學測故事模式目前只做學測（GSAT），會考（CAP）要不要比照做一份？如果要，範圍與流程可以沿用這次的三切片模式（試行→全量生成→App串接）。
- `USER_MANUAL.md`／`DEVELOPER_LOG.md` 還沒補上故事模式章節，要不要近期補上（依 `AGENTS.md` 的「成果文件交付閘」，正式把這個功能標記為完整交付前建議補齊）。
