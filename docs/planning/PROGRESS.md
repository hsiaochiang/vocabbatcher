...
- **目前階段：** M4「品質收斂」全部完成（S1~S2-S4），待負責人最終驗收；M5「學測單字庫」App 端（S2）已實作、學測資料已改用新來源整份重轉並經負責人確認、commit 並 push（`d0cbac1`），品質達 100% 完整率
- **整體進度：** ▓▓▓▓▓▓▓▓▓▓ 100%＋（M1~M4 全部完成；M5 新增里程碑資料與 App 端皆已完成並確認，待 M4/M5 App 端最終驗收動作完成即可視為全部里程碑完成）
...
2. **M5 最終驗收（App 端）**：請依 `docs/handoff/REPORT_M5S2_App端來源切換.md` 的白話步驟測：首頁能切「會考」／「學測」；學測建立批次、翻牌、考試只出學測字；切回會考後看不到學測批次；登入後各考一次會考/學測，確認成績歷史與單字統計會依來源分開顯示。過了這關，M1~M5 全部里程碑就正式完成。
...
- M5「學測單字庫」三片皆已實作完成，待負責人依 M5-S1/S1b/M5-S2 報告完成資料與 App 端驗收
...
- [x] M5-S1/S1b/S2：學測資料管線、OCR 校正、App 端來源切換（已實作完成，待負責人最終驗收）
