...
- **2026-08-14：負責人本機驗證通過，規劃層 `git push` 並跑 `.\deploy.ps1` 部署到 Firebase Hosting（`https://gen-lang-client-0930375434.web.app`），GitHub Pages 備援管道也因 push 自動觸發部署。負責人已在正式站上確認沒問題。學測 Minecraft 故事模式（第16~20頁人工試行 + 第1~15/21~80頁執行層生成，共80頁）正式上線。**
- **2026-08-14：成果文件交付閘補齊**——`USER_MANUAL.md` 新增「七、如何使用故事模式（學測限定）」章節與對應常見問題（原七~十四節依序改為八~十五節）；`DEVELOPER_LOG.md` 新增「1-5 學測 Minecraft 故事模式資料」小節（含資料流、schema、**模板化生成被抓包退回重做的完整教訓**），並更新型別/服務/頁面對照表、PWA 快取規則說明（順手修正一處早就過期的敘述：快取策略文件寫著 `CacheFirst`，實際 2026-08-04 就已經改成 `StaleWhileRevalidate`）、已知問題與決策時間軸。這個功能現在算是完整交付（含成果文件），依 `AGENTS.md`「★ 成果文件交付閘」規則過關。
