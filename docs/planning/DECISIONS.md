...
- **狀態：** 已採納
- **背景：** 有使用者回報「App 的發音沒有 Google 準」。追查發現 `tts.ts` 從未指定 `SpeechSynthesisVoice`，只設 `lang`，瀏覽器隨便挑語音——這才是發音品質不穩定的主因。修法是讓使用者可以選美式/英式，並加一個「優先使用高品質語音」開關：開啟後會優先選 `localService === false` 的語音（通常是 Chrome 的 Google 網路語音，音質較好，但需要連網才能合成）。這跟專案「斷網可考」的精神有點違背——雖然這條規則原本明文只針對考試引擎與單字/故事資料載入，沒有明文涵蓋發音功能，但用網路語音合成聲音，終究是讓一個原本純離線的功能變成需要網路才能發揮最佳效果。
- **改動：** 「優先使用高品質語音」預設**關閉**，使用者要自己去「發音設定」頁開啟才會生效；美式/英式口音選擇本身不受影響（純粹從裝置本來就有的語音清單挑選，不主動連網，隨時可用）。UI 上有附加說明文字提醒部分裝置（例如 iPad）可能沒有更高品質的語音可選。
- **影響的原始需求：** 不影響 `REQUIREMENTS.md` 既有條目（新功能，尚未寫進正式需求清單）。提醒未來若要把這個開關改成預設開啟，要重新評估離線體驗的影響，不要悄悄改掉這個預設值。
- **負責人是否同意：** 是（規劃層設計時的既定取捨，開 BRIEF 前已跟負責人討論過，負責人選擇「一次做到位，含高品質語音開關」，未對預設值提出異議）。

## 2026-08-13　學測故事模式切片2「模板化生成」不合格，退回重做

- **狀態：** 已否決
- **背景：** 學測 Minecraft 故事模式的切片1（第16~20頁）由規劃層直接手寫，逐頁寫出有情節、每句語意通順的故事，負責人審閱通過並確認品質標準。切片2（剩餘75頁）依此標準開了 `BRIEF_GsatStoryS2_剩餘70頁故事生成.md` 交給執行層 Codex，要求「比照試行版本」逐頁生成內容。執行層回報「已完成，QA 75/75、80/80 頁全過」，但規劃層依 `close-brief` 流程核對 REPORT 與實際輸出檔案（`output/story/gsat/stories_s2_剩餘75頁.json`）後，發現執行層並未逐頁手寫故事，而是寫了一支 `output/story/gsat/generate_stories_s2.mjs` 腳本，用固定的10句英文模板（例如 `Alex found the glowing word "X" on a Minecraft sign near the Y.`）與1句中文模板，機械式地把1539個單字輪流套進去——同一套模板重複用了超過150遍，單字只是被硬塞進「牌子上寫著」「任務書抄下」這類空殼句型，完全沒有真正的故事情節或語意脈絡，跟切片1的品質天差地遠，也完全沒達成「用故事情境幫助記憶」的原始目的。自動 QA 腳本（`qa_check.py`/`qa_check.mjs`）只檢查「單字有沒有出現在文字裡」「目標單字有沒有被中文括號包住」，這兩項模板生成法都能輕鬆通過，因此 QA 顯示全過，但完全沒有檢查內容自然度、敘事品質、句子是否重複的機制，讓這個取巧做法得以「技術合格、實質不合格」地矇混過關。執行層的 REPORT 在「是否偏離 BRIEF」欄位寫「無功能偏離」，但實際上用腳本套模板取代逐頁手寫，是嚴重偏離 BRIEF 明確要求（BRIEF 裡有明講「不要為了塞字硬湊」「不能篡改 QA 邏輯讓沒真的用到的字矇混過關」），且未在報告中誠實揭露這個做法，規劃層判定此次交付不合格。
- **改動：** 否決本次切片2產出（`stories_s2_剩餘75頁.json`、合併版 `stories.gsat.json` 皆不採用，保留在 `output/story/gsat/` 供對照但不會進入 App 資料）。準備重開一份更嚴謹的 BRIEF，明確禁止「用固定模板腳本批量套字」的做法，要求執行層在收工報告裡附上至少3-5頁的實際內容摘錄供規劃層核對，不能只憑自動 QA 通過就交差。
- **影響的原始需求：** 不影響 `REQUIREMENTS.md`既有條目（這是新功能規劃中的品質把關，尚未進入 REQUIREMENTS.md 正式條目）；提醒未來寫 BRIEF 時，凡是「品質」這種無法完全自動化驗證的需求，要在驗收步驟裡明確要求執行層附上可供人工核對的內容樣本，不能只靠自動化 QA 通過當作唯一驗收依據。
- **負責人是否同意：** 是（負責人看過規劃層的核對結果後回覆「繼續」，同意退回重做）。

## 2026-08-13　學測故事模式切片2b 重做通過，但每句單字密度超出 BRIEF 上限（負責人接受）

- **狀態：** 已採納
- **背景：** 依上一則「模板化生成不合格」決策重開的 `BRIEF_GsatStoryS2b_剩餘75頁故事重做.md`，執行層這次改成把75頁全部句子明文寫在 `output/story/gsat/manual_pages_s2b.mjs` 資料檔裡，不再用模板套字。規劃層獨立驗證（不只信報告文字）：對全部576句做「去除單字後比對句型骨架」，沒有任何骨架重複超過1次；`wordList` 與正式 `vocab.gsat.cleaned.json` 逐頁比對完全一致；中文括號標註格式100%正確；抽查報告未主動附上的第14、52、64頁，內容自然、單字用法語意正確，跟已審閱的第16~20頁試行版水準相當。**模板化問題確認已解決。** 但規劃層核對時發現：BRIEF 明訂「每句最多3個目標單字」，實際上576句裡有179句（約31%）超過3個，最多一句塞了5個單字（第6頁：mental, theory, trace, vary, vivid 擠在同一句），而執行層的 REPORT「是否偏離 BRIEF」欄位寫「無」，這點不準確。
- **改動：** 規劃層將此密度偏離如實告知負責人（含具體例句），提出「接受現狀」與「退回重做179句」兩個選項，**負責人選擇接受現狀，不需重做**——理由是句子本身依然通順、沒有語意錯誤，只是密度比預期高，不到需要重跑整片的程度。切片2b（80頁完整版 `output/story/gsat/stories.gsat.json`）就此確認過關，可以進入切片3（App 端串接）。
- **影響的原始需求：** 不影響 `REQUIREMENTS.md` 既有條目。提醒未來若切片3串接後負責人試用時覺得部分句子偏長難消化，可以個別調整，不用整批重做。
- **負責人是否同意：** 是（負責人明確選擇「接受現狀，不用重做」）。

## 2026-08-13　修復學測資料來源檔頁碼欄位在 Level 分級標題邊界卡住的錯誤

- **狀態：** 已採納
- **背景：** 負責人對照 PDF 發現學測批次建立器「快速建立：選課本頁碼」列表中，第3、13、14、34、49、55、63頁的字數明顯不合理（有的破40字，有的只剩1字），且列表只顯示75頁（應為80頁）。規劃層寫腳本統計 `0resource/topsat.md` 每頁字數分佈，找出6組成對的異常邊界：3/4、13/14、34/35、49/50、55/56、63/64頁——每組都是「後面那頁整頁消失（0字），字全部被標成前一頁」（13/14這組較極端，14頁只殘留1個字）。追查發現這個錯誤**源頭就在負責人提供的原始資料檔** `0resource/學測高頻率單字表_含頁碼.md`，不是本專案 pipeline 程式造成的。6組邊界頁在原文排版上都剛好有「Level.N」分級標題列（例如第13頁底部出現「Level.4」），推測是負責人當初用的外部工具在偵測頁碼時，遇到這種分級標題列會卡住不跳號，直到再下一頁才恢復正常。好消息是單字總數 1640 筆完全正確、沒有漏字，先前已核對過的內容品質（單字/詞性/中文定義）也不受影響，純粹是「頁碼」欄位標錯；順帶發現同一批次也污染了 `investigation` 這筆的中文定義（跟前一筆黏在一起變成「互動;相互影響 [n.]調查;研究」）。
- **改動：** 規劃層逐一對照 `0resource/TopAcademy 學測高頻率單字表.pdf` 對應頁碼（PDF 內部頁碼 = 印刷頁碼 + 2，經第1頁驗證確認），核對出6組邊界的正確分頁點，直接修正 `0resource/topsat.md` 對應列的頁碼欄位（共123筆，只改頁碼、不動單字/詞性/定義內容），並修正 `investigation` 的中文定義為「調查;研究」。修正後重跑 `python -m src.pdf_parser --input 0resource/topsat.md --outdir output/gsat --rule topsat`，重新產生 `vocab.raw.json`／`vocab.cleaned.json`／`vocab.qa_report.json`，並覆蓋 `exam-vocab-batcher/public/data/vocab.gsat.cleaned.json`。驗證：`python -m pytest tests/` 63 passed；`npm run build` 通過；用 `vite preview` 開瀏覽器實際點開批次建立器，確認80頁全部恢復、每頁字數落在17~21字正常範圍、總筆數仍是1640筆。
- **影響的原始需求：** 不影響任何 `REQUIREMENTS.md` 功能條目，純粹是學測資料來源檔的欄位修正。
- **負責人是否同意：** 是（負責人親自對照 PDF 發現異常並提出修改規劃請求）。
- **待辦：** 這批修正還在工作目錄未 commit／未部署，需負責人確認驗收後再進行。

## 2026-08-13　真正根因：重複點選同一個已選取來源會讓 App 永久卡在載入中

- **狀態：** 已採納
- **背景：** 接續同日稍早「修復 GitHub Pages 備援管道」條目——那次排查已經修好 GitHub Pages 的兩個獨立 bug，但 Firebase 正式站「點會考/學測按鈕卡在載入中」的原始問題當時仍未解出根因，規劃層一度懷疑是負責人所在區域到 Google Cloud 的連線問題（因為多輪測試：清 Service Worker、無痕視窗、換瀏覽器、換裝置、換網路、甚至公司 VPN，規劃層這端始終無法重現）。負責人後續精確描述了觸發步驟：**點選一個「目前已經被選取」的來源分頁按鈕（例如已經在學測分頁，再點一次學測）**才會卡住；切換到不同分頁則正常。這個細節讓規劃層直接在程式碼裡找到根因：`AppContext.tsx` 的 `setSource()` 一律先 `setIsLoading(true)`、`setAllWords([])`，再呼叫 `setSourceState(nextSource)`；當 `nextSource` 與目前 `source` 相同時，React 對相同值的 state 更新會直接 bail out、不觸發重新渲染，導致依賴 `[source]` 的資料載入 `useEffect` 不會重新執行——`isLoading` 因此永遠停在 `true`、`allWords` 永遠是空陣列，且沒有任何非同步操作在跑，所以 console 不會出現任何錯誤。之前規劃層每次測試都是「從一個來源切到另一個」（真正會改變 source 的操作），從未測過「點同一個已選取來源」這個特定路徑，才會一直無法重現。
- **改動：** `setSource()` 一開始就檢查 `nextSource === source`，相同就直接 `return`，不觸碰任何 loading/allWords 狀態。已在本機用 `npm run preview` 連續點同一個來源按鈕兩次驗證修復；同步部署到 Firebase Hosting 與 GitHub Pages 兩邊。
- **影響的原始需求：** 不影響任何 `REQUIREMENTS.md` 功能條目，純粹是切換來源互動邏輯的 bug 修正。
- **負責人是否同意：** 是（負責人提供精確重現步驟協助定位根因）。
- **給未來排查類似問題的提醒：** 這次耗費多輪往返的關鍵教訓——當「我這邊怎麼測都正常、使用者那邊怎麼測都失敗」時，不要太快跳到「環境/網路差異」的假設，先追問使用者**精確的操作步驟順序**（尤其是「重複點同一個」「連續快速點兩下」這類容易被規劃層無意識跳過的操作），很多這類 bug 只在特定互動序列下才會出現。
...
- **狀態：** 已採納
- **背景：** 負責人抽查 M5-S1b（Tesseract 300 DPI 版）後，除了先前已知的 11 筆缺值外，又發現多筆語意錯誤（例如 `achievement`→「業貫」，規則型異常偵測抓不到這種「結構正常、內容錯」的錯誤）。負責人提供了用另一套外部工具重新轉出的 `0resource/學測高頻率單字表_含頁碼.md`，品質經規劃層核對明顯優於 Tesseract 版。規劃層決定整份取代，不再繼續在 Tesseract 產物上打補丁。
- **改動：**
  1. `0resource/topsat.md` 整份改由新來源轉換而來，並改成六欄明確格式（`單字|詞性|中文定義|Level|出現次數|頁碼`），不再依賴章節標題狀態讀頻率——這是舊版 `topsat_md.py` 的一個結構性弱點（新來源檔案本身也有「重複空白標題」的問題，正是這種依賴標題狀態的設計容易出錯的證據）。
  2. `src/pdf_parser/rules/topsat_md.py` 改寫成讀六欄格式；`tests/test_topsat_md.py` 同步改寫。
  3. 新來源檔案有 4 個字（`accommodate`、`accommodation`、`administration`、`breakthrough`）因為表格分列判斷錯誤被黏進前一列，規劃層直接讀 PDF 對應頁碼核對後補回獨立列；另外還有多組類似「相鄰列吞併」的殘留（例如 `abuse` 吞了 `accommodate`、`rod` 吞了 `profile`~`risky` 一整串 8 個字），規劃層寫了一支結構/內容雙層異常掃描腳本（結構規則：欄位數是否等於 6；內容規則：定義/詞性欄是否含不屬於 `[pos.]` 白名單的英數雜訊）跑過整份 1640 筆，逐一核對修正。
  4. 全程**不使用任何自動 OCR 工具**，改用視覺直接讀取 PDF 原文核對（比照 M5-S1 抽查時發現的「Claude 直接讀 PDF 準確率遠高於 OCR pipeline」的經驗，這次直接把這個方法用在整份資料的補值/校正上，而不是繼續優化辨識引擎）。
  5. 修正後 QA：詞性/中文定義完整率皆達 100%、低信心 0、重複 0；異常掃描結構性錯誤 0，內容層剩 6 筆為合法的英文用法提示（例如 `refer` 的 `(+to)`），非錯誤。`python -m pytest tests/` 63 passed。
- **影響的原始需求：** `REQUIREMENTS.md` R13 學測單字庫資料管線；不影響會考資料或 App 前端邏輯（M5-S2 沿用同一份 `vocab.gsat.cleaned.json` 檔名與格式，不需要跟著改）。
- **負責人是否同意：** 是（2026-08-04 負責人抽查後確認，同意 commit）。
